...
def Upload(request):
    if request.method == 'POST':
        ...
        headers = {'Content-Type': 'application/xml'}
        response = requests.post('http://127.0.0.1:5000/cargarPalabras', data=archivo.read(), headers=headers)
        if response.status_code==200:
            print("Correcto")
            return render(request, 'upload.html',{'respuesta_servidor':'Aceptado'})
        else:
            print("incorrecto")
            return render(request, 'upload.html',{'respuesta_servidor':'Incorrecto'})    
    else:
        return render(request, 'upload.html',{'respuesta_servidor':''})